...
function LineGraph() {
  const graphstate = useSelector((state) => state.graphData.OrderTime);
    ...
    <div className="relative w-full graphwrapper h-[300px] max-[1358px]:h-auto">
      <Line
        data={{
          ...
          datasets: [
            {
              label: graphstate.lastSixData21.map((x) => {
                return x.Time;
              }),
              data: graphstate.lastSixData21.map((x) => {
                return x.orders;
              }),
            },
            {
              data: graphstate.lastSixData22.map((x) => {
                return x.orders;
              }),
              borderColor: "#D9E1EC",
              borderWidth: 4,
